...
function shh() { counter.clicks += 1000; }
function displayClicks() {
  return document.querySelector(".counter").innerHTML = `You have clicked me ${counter.clicks} time(s)`;
};
function buyCookie() {
...
function determineClicks() {
  clicksAmount = (cookies + superCookies + redCookies);
}
function buyRedCookie() {
  ...
  determineClicks(); determineRedCookieBonus(); 
  }
  else if (document.querySelector(".redCookie").onclick && counter.clicks >= 25000 && redCookiesAmount === 1) {
    alert("Congrats on getting the red cookie!");
  }
